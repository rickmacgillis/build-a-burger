...
export function* authUserSaga(action) {
    ...
    try {
        ...
        const response = yield axios.post(url + process.env.REACT_APP_INSECURE_FIREBASE_API_KEY, authData);
...
export function* authCheckStateSaga(action) {
    ...
    const expiresIn = yield call([localStorage, 'getItem'], 'expirationDate');
    ...
    if (!token || !userId || expirationDate < new Date()) {
        ...
        yield put(actions.checkAuthTimeout(expirySeconds));
    }
}
